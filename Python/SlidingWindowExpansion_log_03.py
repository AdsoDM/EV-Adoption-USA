...
def main():
    ...
    print("\n--- Sliding Window Analysis (Log Transform) ---")
    ...
    for current_year in prediction_years:
        ...
        # Baseline (on log scale then transformed back)
        mean_log_train = np.mean(y_train_log)
        baseline_pred = np.expm1(mean_log_train)
        
        baseline_rmse = np.sqrt(mean_squared_error(y_test, np.full_like(y_test, baseline_pred)))
        ...
        # Predict (Log Scale)
        log_preds = model.predict(X_test_pca)
        
        # Inverse Transform
        final_preds = np.expm1(log_preds)
        
        model_rmse = np.sqrt(mean_squared_error(y_test, final_preds))
        sliding_rmse_results.append(model_rmse)
        
        print(f"  -> Baseline RMSE: {baseline_rmse:.5f}")
        print(f"  -> Model RMSE:    {model_rmse:.5f}")
    ...
    print("\n=========================================================")
    print("        Sliding Window Log - Final Results             ")
    print("=========================================================\n")
    print(final_df)
    print(f"\nAverage Model RMSE:    {np.mean(sliding_rmse_results):.5f}")
    print(f"Average Baseline RMSE: {np.mean(baseline_rmse_results):.5f}")
    print("=========================================================")
